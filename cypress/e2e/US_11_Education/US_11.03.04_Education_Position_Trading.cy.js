...
describe("Education > Menu Item [Position Trading]", () => {
    ...
    beforeEach(function () {
        ...
        cy.get(selectLanguageUrl[`${ind}`]).eq(1).click({ force: true });
        cy.get('span.js-langName').should('include.text', `${langName}`);

    });

    // langNameList.forEach(() => {
    it(`TC_11.03.04_01 | Education > Menu Item [Position Trading] > Test button [Log in] in the header of ${langName} language`, () => {
    ...
    it(`TC_11.03.04_02 | Education > Menu Item [Position Trading] > Test button [Trade] in the header of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
    ...
    it(`TC_11.03.04_03 | Education > Menu Item [Position Trading] > Test button [Start Trading] in Main banner of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
    ...
    it(`TC_11.03.04_04 | Education > Menu Item [Position Trading] > Test button [Try Demo] in Main banner of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
    ...
    it(`TC_11.03.04_05 | Educations > Menu item [Position trading] > Test buttons [Trade] on Widget "Most traded" of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
    ...
    it(`TC_11.03.04_06 | Education > Menu Item [Position Trading] > Test button [Download on the App Store] in the block "Sign up and trade smart today"  of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
    ...
    it(`TC_11.03.04_07 | Education > Menu Item [Position Trading] > Test button [Get it on Google Play] in the block "Sign up and trade smart today" of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
    ...
    it(`TC_11.03.04_08 | Education > Menu Item [Position Trading] > Test button [Explore Web Platform] in the block "Sign up and trade smart today" of ${langName} language`, () => {
        header.getEducationMenu().realHover();
        basePage.clickPositionTrading();
        bannerBtn.clickExploreWebPlatformLink();
        cy.url().should('eq', links.exploreWebPlatformLink);
        signUp.getFormSignUpOnPaltform().should("be.visible")
         signUp.getHeaderNameSignUpOnPlatform().should('have.text', 'Sign up');
        signUp.getEmailOnPlatform().should('be.visible')
            .and('have.attr', 'type', 'email');
        signUp.getPasswordOnPlatform().should('be.visible')
            .and('have.attr', 'type', 'password');
        signUp.getCheckBoxBySigningUp()
            .should('be.visible');
        signUp.getPolicyLinkOnPlatform().should('be.visible')
            .and('have.text', 'Privacy policy');
        signUp.getBtnContinueOnPlatform()
            .should('be.visible')
            .and("have.text", ' Continue ');
        signUp.getLoginLinkFormOnPlatform().should('be.visible')
            .and('have.text', 'Login');
        cy.visit('https://capital.com/position-trading')
    });
})
